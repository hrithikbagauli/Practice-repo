...
const numResults: Array<number> = []; //this is the detailed way of declaring an array and here we can see that its a generic type because its of type array and contains elements of type number.
const mypromise = new Promise<string>((resolve, reject)=>{ //promise is also a generic type because first of all, its of type promise and second, it resolves or rejects to some type. e.g. a promise might resolve to a string or a number.
    setTimeout(()=>{
